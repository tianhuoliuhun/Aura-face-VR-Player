...
| **v2.0.141** | **MPV 解码器选项止血**：经排查确认 MPV 内核自 v1.0.117 起从未实现（无 libmpv 依赖/so，切换后实际仍是 ExoPlayer 重新绑定），设置面板「MPV 解码器」按钮置灰禁用并注明「开发中」（五语适配），持久化的 MPV 偏好一律回退 EXO，消除「选了没反应」的误导 · MPV decoder option disabled as placeholder (never implemented), persisted MPV preference falls back to EXO |
